...
## エンタープライズクラウド設計レビューゲート

エンタープライズ環境では、単一システムの可用性だけでなく、組織全体の統制、監査、費用負担、
運用継続性を同時に確認する必要があります。設計レビューでは、少なくとも以下を明文化します。

- ランディングゾーン: アカウント/サブスクリプション/プロジェクト分割、管理境界、共有サービス、監査ログ集約先。
- 責任分界: IaaS/PaaS/SaaS、マネージドサービス、委託先、アプリケーションチームの責任範囲。
- ガバナンス: ポリシー、タグ、命名規則、予算、例外承認、変更管理、証跡保存期間。
- セキュリティ: ID連携、特権アクセス、秘密情報、暗号化、ログ、脆弱性対応、インシデント時の連絡経路。
- 可用性/DR: SPOF、リージョン/AZ障害、RPO/RTO、バックアップ、復旧演習、縮退運転、フェイルバック。
- FinOps: 予算アラート、データ転送費、予約/割引、停止忘れ、廃棄手順、コスト配賦。
- IaC運用: plan/apply権限の分離、state保護、差分レビュー、ロールバック、検証環境のTTL。

このゲートは、後続の各パターンを採用するかどうかを判断するための入口です。レビュー結果は、
設計ADR、PR、Runbook、監査証跡のいずれかに残し、未解決リスクを運用開始条件から分離します。
