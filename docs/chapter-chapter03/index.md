...
以降の仕様・価格例は理解のための参考情報です。リージョン/契約/時期により変動するため、最新の情報は各プロバイダーの公式ドキュメント/価格表で要確認です。
...
注記：
- 複数AZの可用性は「AZ障害が独立である」前提で見積もられることが多い一方で、
  制御プレーン、運用、設定ミス、依存サービスなどの共通要因により同時障害が起き得ます。
  SLO/SLA 設計では共通依存も含めて評価してください。

#### 事例2: データベース（自前運用が必要なケース）
...
**選択結果：**
```text
推奨構成（自前DBをEC2で運用する場合）：
- インスタンスタイプ: r6i.xlarge
- OS: Amazon Linux 2023
- ストレージ: gp3 (プロビジョンド IOPS)
- 配置: 複数AZで冗長化（レプリケーション/フェイルオーバー手順は自前で設計）

根拠：
- 高メモリ・CPU 比率
- EBS 最適化
- 可用性/バックアップ/パッチ/監視などの運用は利用者責任になる（必要に応じて運用設計を明記）

補足：
- 高可用性と運用負荷の低減が主目的なら、RDS/Aurora などのマネージドDB（Multi-AZ 機能）を優先検討する
```
