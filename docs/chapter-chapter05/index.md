...
本章は「概念 30% / 実装 50% / 運用 20%」程度のバランスで構成されています。初めて読む際は、まず VPC 設計や三層アーキテクチャといった考え方に集中し、Pythonコードの詳細は後から確認する読み方でも問題ありません。
...
以下のPythonコードは、良い VPC 設計と悪い VPC 設計を比較し、設計時に意識すべきポイントを整理したサンプルです。
