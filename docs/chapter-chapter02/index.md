...
**第一に、レイテンシの最小化**です。光の速度は有限であり、東京からシリコンバレーまでの往復遅延（RTT）は、物理距離と伝搬速度の制約により、理想化した条件でも数十ミリ秒以上になり得ます（実際は経路、媒体、混雑などで変動します）。ユーザー体験を重視するアプリケーションにとって、この遅延は無視できません。
...
AZは、**高可用性と低レイテンシの両立**を可能にします。同一リージョン内のAZ間は高速な専用ネットワークで接続されており、数ミリ秒程度の遅延でデータを同期できます（値はリージョン差や計測方法により変動するため、要件が厳しい場合は実測が必要です）。これにより、リアルタイムのレプリケーションやフェイルオーバーが実現可能です。
...
2つのAZにまたがってシステムを構築し、両方が同時に障害を起こす確率が独立である場合、理論的な可用性は次のとおりです。
...
注記: `0.0.0.0/0` での HTTPS 公開は、インターネット向けの入口を持つ Web 層やロードバランサーの最小例です。管理用ポートやアプリ間通信まで同じ考え方を広げず、可能であれば CloudFront / WAF / CDN / reverse proxy などの前段を経由させた上で、送信元 CIDR や managed prefix list まで絞ってください。
...
注記: human principal の safer default は、IAM User と長期アクセスキーを増やすことではなく、IAM Identity Center または外部 IdP からのフェデレーション + `AssumeRole` です。IAM User は bootstrap や例外運用に限定し、root ユーザーは日常運用に使わない前提で設計してください。
...
注記: この `Action: "*"` / `Resource: "*"` は ABAC の条件式を短く示すための簡略例です。実運用では、`ec2:DescribeInstances` / `ec2:StartInstances` / `ec2:StopInstances` のような具体アクションと、対象リージョン・対象リソース種別の ARN へ絞り、IAM Policy Simulator や Access Analyzer でタグ不一致時に拒否されることまで確認してください。
...
**認証情報の運用**
- 人手による管理アクセスでは長期アクセスキーを常用せず、IdP 連携や `AssumeRole` を使った短期認証情報を優先する
- 緊急用アクセスも、定期棚卸しと利用後の無効化を前提にする
...
- バージョン管理とレビュープロセス
- 環境間の一貫性確保
- 自動化ロールや self-service 用ロールでは `iam:PassRole` を必要なロール ARN に絞り、必要に応じて permissions boundary を併用する
...
AWS などのパブリッククラウドでユーザー定義のコスト配分タグを使う場合は、タグを付けるだけでは請求レポートに反映されません。予算アラートやチャージバックを運用へ載せる前に、請求管理側で対象タグを有効化し、主要リソースのタグ付与率と反映完了を確認してください。
...
IAM ポリシーでタグを条件として使用し、詳細なアクセス制御を実現できます。
